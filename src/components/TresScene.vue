...
const textureDir = `${process.env.BASE_URL}textures/earth/${isMobile ? 'low-res/' : ''}`

// Note: textures CANNOT be loaded in the setup script
const earthTexture = await useTexture({
  map: `${textureDir}diffuse.jpg`,
  displacementMap: `${textureDir}spec-inverted.jpg`,
});

const earthCloudTexture = await useTexture({
  alphaMap: `${textureDir}clouds.jpg`,
});
